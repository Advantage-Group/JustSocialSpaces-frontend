...
const SignupForm = ({ onClose }) => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${config.API_BASE_URL}/auth/set-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  if (step === 1) {
    ...
    return (
      <div className="signup-modal-centered">
        <button className="signup-close" aria-label="Close" onClick={onClose}>
          ×
        </button>
