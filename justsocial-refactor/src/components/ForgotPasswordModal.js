...
function ForgotPasswordModal({ onClose }) {
  ...
  const handleCodeSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `${config.API_BASE_URL}/auth/verify-confirmation-code`,
        {
          email: value,
          code: code,
        },
  ...
  return (
    <div className="login-overlay">
      <div className="login-modal">
        <button className="login-close" aria-label="Close" onClick={onClose}>
          ×
        </button>
        <div className="login-logo">
          <svg
            className="x-logo-img"
            width="40"
            height="40"
            viewBox="0 0 40 40"
            fill="none"
          ...
          >
            <g>
              <path
                d="M32.5 7.5H28.75L20 18.75L11.25 7.5H7.5L18.75 22.5L7.5 37.5H11.25L20 26.25L28.75 37.5H32.5L21.25 22.5L32.5 7.5Z"
                fill="white"
              />
            </g>
          </svg>
        ...
        {step === "email" && (
          <>
            ...
            <button
              className="login-btn next forgot-next"
        ...
        {step === "code" && (
          <>
            ...
            <div className="login-desc">
              We sent a confirmation code to <b>{maskedEmail}</b>. Enter it
              below to continue.
            </div>
            <input
