...
const PostDetailModal = ({ isOpen, onClose, post }) => {
  ...
  const fetchReplies = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.API_BASE_URL}/posts/${post._id}/comments`,
        {
          headers: {
  ...
  const handleReply = async () => {
    if (
      (!replyText.trim() && selectedMedia.length === 0 && !selectedGif) ||
      isPostingReply ||
      !post?._id
    )
      return;
  ...
  const handleLike = async () => {
    ...
    if (isLiked) {
      ...
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `${config.API_BASE_URL}/posts/${post._id}/react`,
          {
            method: "DELETE",
    ...
    <div className="post-detail-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="post-detail-modal-header">
          ...
          <button className="post-detail-modal-nav-btn" aria-label="Forward">
            <svg
              viewBox="0 0 24 24"
              fill="currentColor"
              style={{ width: "20px", height: "20px" }}
        ...
        <div className="post-detail-modal-content">
          ...
          <div className="post-detail-modal-main">
            <div className="post-detail-modal-main-inner">
              ...
              <article className="post-detail-main-post">
                ...
                <div className="post-detail-user-info">
                  <img
                    src={
                      post.author?.photo
                        ? `${post.author.photo}${post.author.photo.includes("?") ? "&" : "?"}v=${post.author?._id || post.author?.id || "default"}`
                    ...
                    onLoad={() => {
                      console.log(
                        "Post detail avatar loaded successfully:",
                        post.author?.photo,
                      );
                    }}
                    onError={(e) => {
                      console.error(
                        "Post detail avatar failed to load:",
                        post.author?.photo,
                      );
                ...
                {(post.media?.length > 0 ||
                  ...
                  <div className="post-detail-post-media">
                    {post.gif ? (
                      <img src={post.gif.url} alt={post.gif.title || "GIF"} />
                    ) : post.media?.length > 0 ? (
              ...
              <div
                style={{ padding: "16px", borderBottom: "1px solid #2f3336" }}
              >
                <h3
                  style={{
                    fontSize: "20px",
                ...
                <div className="post-detail-reply-section">
                  <img
                    src={
                      state.user?.photo
                        ? `${state.user.photo}${state.user.photo.includes("?") ? "&" : "?"}v=${state.user?._id || state.user?.id || "default"}`
                        : `https://ui-avatars.com/api/?name=${encodeURIComponent(state.user?.name || "User")}`
                    }
                    alt="Your profile"
                    ...
                    onError={(e) => {
                      console.error(
                        "Reply input avatar failed to load:",
                        state.user?.photo,
                      );
                    ...
                    {selectedMedia.length > 0 && (
                      <div style={{ marginBottom: "12px" }}>
                        {selectedMedia.map((media, index) => (
                          <div
                            key={index}
                            className="post-detail-reply-media-preview"
                          >
              ...
              <div style={{ padding: "16px" }}>
                ...
                {loadingReplies ? (
                  ...
                  <div ref={repliesRef}>
                    {replies.map((reply) => {
                      const replyTextParts = formatPostText(
                        reply.content || "",
                      );
                      return (
                        <div key={reply._id} className="post-detail-reply-item">
                          <div className="post-detail-reply-header">
                            <img
                              src={
                                reply.author?.photo
                                  ? `${reply.author.photo}${reply.author.photo.includes("?") ? "&" : "?"}v=${reply.author?._id || reply.author?.id || "default"}`
                                  : `https://ui-avatars.com/api/?name=${encodeURIComponent(reply.author?.name || "User")}`
                              }
                            ...
                            <div className="post-detail-reply-content">
                              <div className="post-detail-reply-meta">
                                <span className="post-detail-reply-author-name">
                                  {reply.author?.name || "User"}
                                </span>
                                <span className="post-detail-reply-author-handle">
                                  @
                                  {reply.author?.email?.split("@")[0] || "user"}
                                </span>
                                <span className="post-detail-reply-time">
