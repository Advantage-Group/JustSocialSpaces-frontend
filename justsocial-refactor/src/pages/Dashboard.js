...
const Dashboard = () => {
  ...
  useEffect(() => {
    const handleProfilePhotoUpdate = (event) => {
      ...
      console.log(
        "Dashboard: Profile photo updated event received",
        event.detail,
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.posts && data.posts.length > 0) {
          console.log(
            "Dashboard - First post author photo:",
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (pollData) {
        ...
        console.log(
          "Dashboard - Final pollDataWithQuestion:",
          pollDataWithQuestion,
        );
        console.log(
          "Dashboard - Sending poll data to backend:",
          requestBody.poll,
        );
      }
  ...
  const fetchSuggestedUsers = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${config.API_BASE_URL}/auth/suggested-users?limit=5`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleFollowUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${config.API_BASE_URL}/auth/follow/${userId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleUnfollowUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${config.API_BASE_URL}/auth/unfollow/${userId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <div className="user-profile-container">
          ...
          >
            <img
              ...
              src={
                user?.photo
                  ? `${user.photo}${user.photo.includes("?") ? "&" : "?"}v=${Date.now()}`
              ...
              onError={(e) => {
                console.error(
                  "Dashboard sidebar avatar failed to load:",
                  user?.photo,
                );
                e.target.src =
                  "https://ui-avatars.com/api/?name=" +
      ...
      <div className="center-column">
        ...
        <div className="compose-post">
          <div className="compose-header">
            <img
              ...
              onError={(e) => {
                console.error(
                  "Dashboard compose avatar failed to load:",
                  user?.photo,
                );
                e.target.src =
                  "https://ui-avatars.com/api/?name=" +
            ...
            <div className="compose-input-container">
              <textarea
                ...
                placeholder={
                  showPollCreator ? "Ask a question..." : "What is happening?!"
                }
                value={newPost}
              ...
              {/* Debug info */}
              <div
                style={{ color: "#71767b", fontSize: "12px", marginTop: "4px" }}
              >
              ...
              <div className="compose-actions">
                <div className="compose-icons">
                  ...
                  <button
                    ...
                    onClick={() => {
                      console.log(
                        "Poll button clicked, current state:",
                        showPollCreator,
                      );
                      ...
                      console.log(
                        "showPollCreator state updated to:",
                        newState,
                      );
                    }}
                    disabled={isPosting}
                ...
                >
                  ...
                  {console.log(
                    "Post button state - newPost:",
                    newPost,
                    "pollData:",
                    pollData,
                    "disabled:",
                    (!newPost.trim() && !pollData) || isPosting,
                  )}
                </button>
              </div>
            </div>
        ...
        {showPollCreator && (
          <div className="poll-creator-integrated">
            ...
            >
              <h3 style={{ color: "#1d9bf0", margin: "0 0 16px 0" }}>
                Poll Creator
              </h3>
              <PollCreator
              ...
              if (activeTab === "following") {
                return (
                  <div className="no-posts">
                    ...
                    <h3>No posts yet</h3>
                    <p>
                      When people you follow post, their posts will show up
                      here.
                    </p>
                  </div>
                );
              ...
              <div key={post._id} className="post-card">
                <div className="post-header">
                  <img
                    ...
                    onError={(e) => {
                      console.error(
                        "Dashboard post avatar failed to load:",
                        post.author?.photo,
                      );
                      e.target.src =
                        "https://ui-avatars.com/api/?name=" +
                ...
                {post.poll && (
                  <>
                    {console.log(
                      "Dashboard - Rendering poll for post:",
                      post._id,
                      "poll data:",
                      post.poll,
                    )}
                  ...
                  <div className="post-reactions">
                    {(() => {
                      ...
                      (post.reactions || []).forEach((r) => {
                        if (!byEmoji[r.emoji])
                          byEmoji[r.emoji] = { count: 0, self: false };
                        byEmoji[r.emoji].count++;
                        if (r.userId === (user?._id || user?.id))
                          byEmoji[r.emoji].self = true;
                      });
                        ...
                        <div className="reactions-container">
                          {emojis.map(([emoji, info]) => (
                            <button
                              ...
                              onClick={() =>
                                info.self
                                  ? handleRemoveReaction(post._id)
                                  : handleAddOrChangeReaction(post._id, emoji)
                              }
                              style={{
                                fontSize: 20,
                                background: info.self ? "#1d9bf0" : "#23272c",
                                borderRadius: 18,
                                padding: "2px 10px",
                                border: "none",
                                cursor: "pointer",
                                ...
                                outline: info.self
                                  ...
                                  : "none",
                                opacity:
                                  reactionLoadingPostId === post._id ? 0.6 : 1,
                              }}
                              title={
                                info.self ? "Remove your reaction" : "React"
                              }
                            >
                              {emoji}{" "}
                              {info.count > 1 && (
                          ...
                          <button
                            ...
                            disabled={reactionLoadingPostId === post._id}
                            onClick={() =>
                              setShowReactionPickerPostId(post._id)
                            }
                          >
                            <svg viewBox="0 0 24 24" width="22" height="22">
                              <path
                                d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm-3 7a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm6 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM12 18c-2.137 0-3.998-1.157-4.9-2.889-.126-.242.06-.535.332-.535h9.136c.272 0 .458.293.332.535C15.998 16.843 14.137 18 12 18z"
                                fill="currentColor"
                              />
                            </svg>
                          </button>
      ...
      <div className="right-sidebar">
        ...
        <div className="who-to-follow">
          ...
          {suggestedUsers.length > 0 ? (
              ...
              return (
                <div key={suggestedUser._id} className="follow-suggestion">
                  <img
                    src={
                      suggestedUser.photo ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(suggestedUser.name || "User")}`
                    }
                  ...
                  >
                    ...
                    <span className="suggestion-handle">
                      @{suggestedUser.email?.split("@")[0] || "user"}
                    </span>
                  </div>
                  <button
            ...
            <div
              style={{ padding: "12px 0", color: "#71767b", fontSize: "15px" }}
            >
      ...
      <div className="mobile-bottom-nav">
        ...
        >
          <img
            ...
            onError={(e) => {
              console.error(
                "Dashboard mobile avatar failed to load:",
                user?.photo,
              );
              e.target.src =
                "https://ui-avatars.com/api/?name=" +
