...
const Messages = () => {
        ...
        setMessages((prev) => {
          ...
          return [
            ...prev,
            {
              ...data.message,
              isOwn:
                (data.message.sender?.id || data.message.sender?._id) ===
                state.user?._id,
            },
          ];
        });
        ...
        (response) => {
          if (response?.ok) {
            ...
            socketRef.current.emit(
              ...
              (response) => {
                if (response?.ok) {
                  const messagesWithOwn = (response.messages || []).map(
                    (msg) => ({
                      ...msg,
                      isOwn:
                        (msg.sender?.id || msg.sender?._id) === state.user?._id,
                    }),
                  );
  ...
  const fetchConversations = async () => {
    try {
      ...
      const response = await fetch(
        `${config.API_BASE_URL}/messages/conversations`,
        {
          headers: {
  ...
  const fetchMessages = async (conversationId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.API_BASE_URL}/messages/conversations/${convId}/messages`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const searchUsersFunc = async (query) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${config.API_BASE_URL}/auth/search-users?q=${encodeURIComponent(query)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const startConversation = async (userId) => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response
          .json()
          .catch(() => ({ error: "Failed to start conversation" }));
        showNotification(
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      for (const file of files) {
        ...
        const response = await fetch(
          `${config.API_BASE_URL}/messages/upload-attachment`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleDownloadAttachment = async (attachment) => {
    try {
      ...
      link.download =
        attachment.filename ||
        `${attachment.type || "file"}-${Date.now()}.${fallbackExt}`;
  ...
  return (
    <div className="messages-page">
      <div className="messages-content">
        {/* Conversations List */}
        <div
          className={`conversations-panel ${isMobile && selectedConversation ? "hidden-mobile" : ""}`}
        >
          {/* Messages Header */}
                ...
                >
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(otherParticipant.name || "User")}&size=48`
                    }
                  ...
                  .map((otherParticipant) => (
                    <React.Fragment key={otherParticipant.id}>
                      <img
                        src={
                          otherParticipant.photo ||
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(otherParticipant.name || "User")}&size=32`
                        }
                        alt={otherParticipant.name}
                ...
                selectedConversation.participants
                  ...
                  .map((otherParticipant) => (
                    <div
                      ...
                      className="chat-user-profile-section"
                    >
                      <div className="chat-user-profile-content">
                        <img
                          src={
                            otherParticipant.photo ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(otherParticipant.name || "User")}&size=80`
                          }
                          alt={otherParticipant.name}
                        ...
                        />
                        <h3 className="chat-user-profile-name">
                          {otherParticipant.name}
                        </h3>
                        {otherParticipant.email && (
                          <p className="chat-user-profile-handle">
                            @{otherParticipant.email.split("@")[0]}
                          </p>
                        )}
                        {otherParticipant.createdAt && (
                          <p className="chat-user-profile-join-date">
                            {formatJoinDate(otherParticipant.createdAt)}
                          </p>
                        )}
                        <button
                          className="chat-user-profile-view-btn"
                          onClick={() => {
                    ...
                    .map((otherParticipant) => (
                      ...
                      >
                        <img
                          src={
                            otherParticipant.photo ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(otherParticipant.name || "User")}&size=100`
                          }
                          alt={otherParticipant.name}
                        ...
                        <div className="profile-summary-handle">
                          @{otherParticipant.email?.split("@")[0]}
                        </div>
                        <div className="profile-summary-joined">
                          {formatJoinDate(otherParticipant.createdAt)}
                        </div>
                        <button
                        ...
                        {!message.isOwn && (
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(message.sender?.name || "User")}&size=32`
                            }
                                            ...
                                            <img
                                              ...
                                              onError={(e) => {
                                                ...
                                                errorDiv.style.cssText =
                                                  "padding: 20px; text-align: center; color: #71767b; background: #202327; border-radius: 12px; border: 1px solid #2f3336;";
              ...
              <div className="message-input-container">
                ...
                <div className="message-input-wrapper">
                  <div className="message-actions-left">
                    ...
                    >
                      <svg viewBox="0 0 24 24">
                        <path d="M8 17c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm8 0c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"></path>
                      </svg>
                    </button>
