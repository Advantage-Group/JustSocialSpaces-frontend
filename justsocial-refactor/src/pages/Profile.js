  ...
  const fetchProfileData = async () => {
    try {
      ...
      if (userId) {
        ...
        const profileRes = await fetch(`${config.API_BASE_URL}/auth/profile`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
      ...
      if (userIdToFetch) {
        const postsRes = await fetch(
          `${config.API_BASE_URL}/posts/user/${userIdToFetch}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const fetchLikedPosts = async () => {
    try {
      ...
      const response = await fetch(
        `${config.API_BASE_URL}/posts/liked/${currentUserId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (uploadRes.ok) {
        ...
        setPosts((prevPosts) => {
          if (prevPosts && prevPosts.length > 0 && currentUserId) {
            ...
            console.log(
              "Updated local posts with new profile photo:",
              updated.length,
              "posts",
            );
            return updated;
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Revert on error
        setPosts((prevPosts) =>
          prevPosts.map((p) => (p._id === postId ? currentPost : p)),
        );
      }
    } catch (e) {
      // Revert on error
  ...
  return (
    <div className="profile-page">
      ...
      <div
        ...
        onClick={() => isOwnProfile && handleEditPhoto("banner")}
      >
      ...
      <div className="profile-info-section">
        <div className="profile-avatar-container">
          <div
            ...
            onClick={() => isOwnProfile && handleEditPhoto("avatar")}
          >
            <img
              key={`profile-avatar-${profileUser?._id || profileUser?.id}-${profileUser?.photo || "default"}-${profileUser?.photo ? profileUser.photo.split("?")[0] : ""}`}
              src={
                profileUser?.photo
                  ? `${profileUser.photo}${profileUser.photo.includes("?") ? "&" : "?"}v=${Date.now()}`
                  : `https://ui-avatars.com/api/?name=${encodeURIComponent(profileUser?.name || "User")}&size=133`
              }
              alt="Profile"
              onError={(e) => {
                console.error(
                ...
                if (
                  profileUser?.photo &&
                  profileUser.photo.includes("/api/posts/media/")
                ) {
                  const testUrl = profileUser.photo
                    .replace("/api/posts/media/", "/api/posts/media-test/")
                    .split("?")[0];
          ...
          return filteredPosts.map((post) => (
            ...
            >
              <img
                ...
                src={
                  post.author?.photo
                    ? `${post.author.photo}${post.author.photo.includes("?") ? "&" : "?"}v=${Date.now()}`
              ...
              <div className="post-content-wrapper">
                ...
                {post.images && post.images.length > 0 && (
                  <div
                    className={`post-images grid-${Math.min(post.images.length, 4)}`}
                  >
                ...
                {post.gif && (
                  <div className="post-gif">
                    <img
                      src={
                        typeof post.gif === "string" ? post.gif : post.gif.url
                      }
