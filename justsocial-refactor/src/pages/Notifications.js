...
const Notifications = () => {
  ...
  const markAsRead = async (notificationId) => {
    try {
      const response = await fetch(
        `${config.API_BASE_URL}/notifications/${notificationId}/read`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const markAllAsRead = async () => {
    try {
      const response = await fetch(
        `${config.API_BASE_URL}/notifications/mark-all-read`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteNotification = async (notificationId) => {
    try {
      const response = await fetch(
        `${config.API_BASE_URL}/notifications/${notificationId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (notification.post && notification.post._id) {
      ...
      (notification.type === "space" ||
        notification.type === "space_joined" ||
        notification.type === "space_invitation" ||
        notification.type === "space_request") &&
      notification.space
    ) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          window.dispatchEvent(
            new CustomEvent("openSpace", { detail: data.space }),
          );
        } else {
          navigate("/dashboard");
        }
                ...
                <div className="notification-avatar">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(notification.fromUser?.name || "User")}`
                    }
                ...
                <div className="notification-text">
                  ...
                  {notification.space && (
                    <div className="notification-space-preview">
                      ...
                      <span>
                        {notification.space.title || "Untitled Space"}
                      </span>
                    </div>
                  )}
